...
let askBing (config: BotConfig) (chatId: int64) (userId: int64) (inputId: int64) (input: string): unit =
    ...
    task {
        let resultMessageHuh = sendToTelegram config (Api.sendMessageReply chatId "⏳ Hold on, let me think about it." inputId) |> processResultWithValue
        ...
        try
            ...
            let! conversation = match funLocalCache.GetConvo chatId userId with
            ...
            sendToTelegram config (Req.EditMessageText.Make(text = "⌛", chatId = ChatId.Int(resultMessage.Chat.Id), messageId = resultMessage.MessageId))
            |> processResult
            1. |> TimeSpan.FromSeconds |> waitFun
            sendToTelegram config (Req.DeleteMessage.Make(chatId = ChatId.Int(resultMessage.Chat.Id), messageId = resultMessage.MessageId))
            |> processResult
        with e ->
